...
int transform(ImageData *img, ImageData *outimg)
{
	...
	for(y = y1, i = 0; y <= y2; y++) {
		for(x = x1; x <= x2; x++) {
			/*
			original: fx = (double)(x - x1) * w1 / (double)w2 + rx1;
			left: fx = (double)(x - x1) * (w1 + x2) / (double)(w2 + x) + rx1;
			right: fx = (double)(x - x1) * (w1 + x) / (double)(w2 + x2) + rx1;
			*/
			fx = (double)(x - x1) * (w1 + x - i) / (double)(w2 + x2 + i) + rx1;
